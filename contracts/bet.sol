...
constructor(string memory _assetName, uint  _price, uint bettingTime) payable {
...
function createPrice(string memory _assetName, uint _price, bool _greaterThan) public {
...
function createBet (string memory _assetName, uint _priceId) external payable {       
    ...
    require (numberOfBetsPerAddress[msg.sender] == 0, "Sorry, you have already placed a bet");
...
function betWinDistribution(uint _priceId) public payable  {
        
    uint div;

    if (_priceId == 0) {
        for (uint i = 0; i < bets.length; i++) {
            if (keccak256(abi.encodePacked((bets[i].priceBet.assetName))) == keccak256(abi.encodePacked("greaterThanBetters"))) {
        ...
        for (uint i = 0; i < bets.length; i++) {
            if (keccak256(abi.encodePacked((bets[i].priceBet.assetName))) == keccak256(abi.encodePacked("lessThanBetters"))) {
                address payable receiver = payable(bets[i].adrs);
    ...
    for (uint i = 0; i < bets.length; i++) {
        numberOfBetsPerAddress[bets[i].adrs] = 0;
    }
